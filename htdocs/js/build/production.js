...
var _deviceToggle = require('./modules/deviceToggle');

var _deviceToggle2 = _interopRequireDefault(_deviceToggle);

var _wayView = require('./modules/wayView');

var _wayView2 = _interopRequireDefault(_wayView);
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.setClasses = function (e) {
    e.preventDefault();
    var t = _this.elements;
    var o = _this.options;
  ...
  this.animateOpen = function (a) {
    ...
    function applyBlur() {
      TweenMax.set(t.deviceSide[0], { webkitFilter: 'blur(' + blurElem.a + 'px)', filter: 'blur(' + blurElem.a + 'px)' });
    }; // function for blur timeline
    newin
    //.to(t.pattern[0], 1, {scale:1.1}, "newin") // zoom pattern
    .from(t.prodInfo[0], .5, { autoAlpha: 0, top: "-=200" }, "newin") // fade new stuff in
    .from(t.deviceSide[0], .75, { top: "-=200" }, "newin").from(t.description[0], .75, { top: "-=140%" }, "newin") // move title down
    .from(t.bigtitle[0], 1, { top: "-60%" }, "newin") // move title down
    .set(t.clicked[0], { background: '#fff', boxShadow: 'none' }, "newin").to(t.clicked[0], .25, { width: btnwidth, height: btnheight, ease: Expo.easeOut }, "newin+=.5"); // change button width


    openTL.to(window, .5, { scrollTo: { y: pos } }, "sayingOut") // scroll to top of clicked div
    ...
    .set(t.prodCont[0], { className: "+=active" }).add(newin, "-=.75").from(t.deviceSide[0], 1, { scaleX: 1.1, scaleY: 1.1 }, "fadeoutin-=1").to(blurElem, 1.25, { a: 0, onUpdate: applyBlur }, "fadeoutin-=1").from(t.button[0], .5, { opacity: 0, ease: Sine.easeOut }, "fadeoutin-=.25") // fade solid button to see through text
    .to(t.sectionTitle[0], .5, { opacity: 0, ease: Sine.easeOut }, "fadeoutin-=.25") // fade solid button to see through text

    ;
    _this.elements.openTL = openTL;
    _this.elements.openTL.play().timeScale(1.5);
  };

  this.closeListen = function () {
    _this.elements.close.on('click', _this.animateClose); // event listener for back click (or scroll)
    _this.elements.body.bind('mousewheel', _this.bounceEffect);
    // add hammer event listener
  };

  this.bounceEffect = function (e) {
    var scro = e.originalEvent.wheelDelta / 4,
        t = _this.elements;
    ...
    if (scro < 0) {
      ...
      if (scro < -55) {
        _this.animateClose(e);
        _this.elements.body.unbind('mousewheel', _this.bounceEffect);
      }
    }
  };

  this.animateClose = function (e) {
    var t = _this.elements;

    e.preventDefault();
    var removestuff = function removestuff() {
      t.prodInfoWrap.remove();
      t.prodInfo.attr('style', ''); // remove gsap inline style
      t.body.css('overflow', 'auto'); // double tap that body
    };

    _this.elements.openTL.reverse(); // scroll to top of clicked div

    _this.elements.openTL.eventCallback("onReverseComplete", removestuff);
  };

  this.plusClick = function (e) {
    var t = _this.elements;
    var o = _this.options;
    _this.setClasses(e);
    _this.constructPage();
...
var _class = function () {
  function _class(options) {
    ...
    this.staticHeights = function () {
      var bodyh = screen.height;
      var toolbar = bodyh - $(window).outerHeight();
      var setHeight = function setHeight(o) {
        var h = bodyh;
        o.height(h);
      };
      var setHeight2 = function setHeight2(o) {
        var h = o.outerHeight();
        o.height(h);
      };
      var setTop = function setTop(o) {
        var t = parseInt(o.css('top')) + toolbar / 2;
        o.css("top", t);
      };
...
var _class = function () {
  function _class(options) {
    ...
    this.classViewToggle = function () {
      var sections = _this.elements.sections;
      var preRender = 2; // number of sections to pre-paint

      sections.each(function (i) {
        // loop sections and hide or show based on index
        if (i > preRender) {
          $(this).addClass('hollow');
        }
        var sect = $(sections); // What to toggle
        var num = preRender + 1; // Offset for array

        var g = 1;

        var up = new Waypoint({ // Up function
          element: this,
          handler: function handler(direction) {
            sect.filter('.active').removeClass('active'); // run this no matter what
            if (direction == 'up') {
              var next = i > sections.length - num ? 1 : i + num; // if we're not the last guy, hide
              var prev = i - preRender; // offset num for array
        ...
        var down = new Waypoint({
          element: this,
          handler: function handler(direction) {
            var prev = i >= num ? i - num : sections.length; // make sure there is a previous
            var next = i + preRender; // offset num for array
            if (direction == 'down') {
